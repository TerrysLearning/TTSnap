        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # here we return both the latent and twediee
                latents, x0_pred = self.scheduler.step(noise_pred, t, latents, **extra_step_kwargs, return_dict=False)
        ...
        if not return_dict:
            ...
            for x0_pred in x0_pred_list:  
                latents_for_vae = x0_pred.to(vae_dtype)

                if hasattr(self.vae.config, "latents_mean") and self.vae.config.latents_mean is not None and \
                hasattr(self.vae.config, "latents_std") and self.vae.config.latents_std is not None:
                    latents_mean = torch.tensor(self.vae.config.latents_mean).view(1, 4, 1, 1).to(latents_for_vae.device, vae_dtype)
                    latents_std = torch.tensor(self.vae.config.latents_std).view(1, 4, 1, 1).to(latents_for_vae.device, vae_dtype)
                    latents_to_decode = latents_for_vae * latents_std / self.vae.config.scaling_factor + latents_mean
                else:
                    latents_to_decode = latents_for_vae / self.vae.config.scaling_factor

                image = self.vae.decode(latents_to_decode, return_dict=False)[0]
                image = self.image_processor.postprocess(image, output_type=output_type)
                x0_out_list.append(image)
            return x0_out_list
